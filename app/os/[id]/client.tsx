...
export function OSPageClient({ os }: OSPageClientProps) {
  ...
  const [filteredGroups, setFilteredGroups] = useState<readonly ShortcutGroupType[]>(os.groups);
  const [platform, setPlatform] = useState<"windows" | "mac">(
    os.platforms.includes("windows") ? "windows" : "mac"
  );
  ...
  const isWindowsAvailable = os.platforms.includes("windows");
  const isMacAvailable = os.platforms.includes("macos");
  ...
  return (
    <div className="py-5">
      ...
      <div className="flex items-start gap-6 mb-8">
        <div className="relative w-16 h-16 shrink-0 bg-gray-100 rounded-lg p-1.5">
          <img src={os.icon} alt={os.name} className="w-full h-full object-contain" />
        </div>
        <div className="flex-1">
          <h1 className="text-3xl font-bold tracking-tight mb-1">{os.name}</h1>
          <p className="text-gray-500">{os.description}</p>
